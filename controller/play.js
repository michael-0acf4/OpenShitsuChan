...
router.get ('/', async (req, res) => {
	...
	if (req.session.user && req.session.user.chara_in_mind) {
		...
		if (gtrace == req.session.user.random_seed && chara_in_mind.probability > accepting_prob_update) {
			let con = null;
			try {
				con = Connection ();
				const name = chara_in_mind.name;
				await con.query ('BEGIN');
				await AnsServices.submitAnswersFor (con, name, '@me', req.session.user.answers);
...
router.get ('/game', async (req, res) => {
	let [error, message] = ['', ''];
	let con = null;
	const game_data = {question: null, responses : QuestServices.getChoicesMap (), ai_reaction_index : 0, number_q : 0};
	try {
		con = Connection ();
		// console.log(req.session.user);
		const exclude = Object.keys (req.session.user.answers); // question ids
		const question = await QuestServices.getSingleQuestion (con, exclude, req.session.user.chara_in_mind);
...
router.get ('/game/choose/:qid/:choice_index', async (req, res) => {
	...
	try {
		...
		const result = await CoreServices.computeProbForAllCharacters (
			con, req.session.user.answers, sort, top
		);
		// update session
		req.session.user.ai_reaction_index = QuestServices.getAIReactionIndex (result);
		...
		if (count_responses >= min_todo && QuestServices.shouldProbablyStopGiven (result)) {
...
router.post ('/game/result/wrong', async (req, res) => {
	...
	try {
		...
		await con.query ('BEGIN');
		await AnsServices.submitAnswersFor (con, name, your_name, req.session.user.answers);
		await con.query ('COMMIT');
