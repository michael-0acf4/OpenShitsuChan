...
module.exports = class BackServices {
  ...
  static async getCharacterByName(con, name) {
    const sql = {
      text:
        `SELECT * FROM "Character" WHERE LOWER(name) LIKE LOWER($1) LIMIT 10`,
      values: ["%" + name + "%"],
    };
  ...
  static async getQuestionByContent(con, content) {
    const sql = {
      text:
        `SELECT * FROM "Question" WHERE LOWER(content) LIKE LOWER($1) LIMIT 10`,
      values: ["%" + content + "%"],
    };
    const res = await con.query(sql);
    return res.rows || [];
  ...
  static async insertOrUpdateCharaFromPost(connexion, character, questions) {
    ...
    for (let question of questions) {
      ...
      if (isNaN(question.probability)) {
        throw Error(
          'Question "' + question.value + '" probability is not a number',
        );
      }
    }
    ...
    for (let question of questions) {
      ...
      await BackServices.insertOrUpdateAnswerFor(
        connexion,
        question.id,
        character.id,
   ...
   */
  static async insertOrUpdateAnswerFor(
    connexion,
    idquestion,
    idcharacter,
    probability,
    ...
    if (answer == null) {
      ...
      logger.warn(
        "Updating answer " + answer.idanswer + " new proba => " + probability,
      );
    }
  }
  ...
  static formatStatString(list_q, total_entropy, total_charas) {
    ...
    str += "Total questions related to at least 1 character : " +
      list_q.length + "\n";
    ...
    for (let q of list_q) {
      ...
      str += "> " + q.sharedWith +
        " chr(s) is/are strongly related to this question\n";
      str += "\n";
    }
    return str;
  }
