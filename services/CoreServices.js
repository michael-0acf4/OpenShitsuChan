...
module.exports = class CoreServices {
     ...
     */
    static async computeProbabilities (connexion, idcharacter, answers_so_far, statCharaOf = null) {
        // {idchara : {a1 : ..., a2 : ...., ...}}
        ...
        return (P_H * prod_pEk_given_H) / makeZeroSafe(P_H * prod_pEk_given_H + P_notH * prod_pEk_given_notH);
    }
     ...
     */
    static async computeProbForAllCharacters (connexion, answers_so_far, sort = true, top = 3) {
        const statCharaOf = await CoreServices.getCharacterSetAsMap (connexion);
        let result = [];
        for (let id in statCharaOf) {
            const probability = await CoreServices.computeProbabilities (connexion, id, answers_so_far, statCharaOf);
            result.push({
                id : id, 
    ...
    static async getCharacterSetAsMap (connexion) {
        ...
        for (let item of result.rows) {
            ...
            map[item.idcharacter].quest[item.idquestion] = parseFloat(item.probability);
        }
        return map;
    }
