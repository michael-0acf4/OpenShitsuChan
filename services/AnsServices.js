...
module.exports = class AnsServices {
     ...
     */
    static async submitAnswersFor (connexion, chara_name, your_name, answers_so_far) {  
        let match_chara = await CoreServices.getBestMatchCharacterByName (connexion, chara_name);
        ...
        for (let qid in answers_so_far) {
            ...
            await AnsServices.insertOrUpdate (connexion, match_chara, qid, pr, chara_name);
        }
    }
     ...
     * @param {string} chara_name 
     */
    static async insertOrUpdate (connexion, character, idquestion, probability, chara_name = 'X') {
        const db = DBModel.use (connexion);
        // chara exists already
        const answer = await AnsServices.getAnswerByIdQuestionAndChara (connexion, idquestion, character.idcharacter);
        if (answer == null) {
            ...
            const nproba = AnsServices.correctProbability (
                parseFloat(answer.probability), probability
            );
     ...
     */
    static async getAnswerByIdQuestionAndChara (connexion, idquestion, idcharacter) {
        const db = DBModel.use (connexion);
        const ans = await db.get_where ('"Answer"', {idquestion : idquestion, idcharacter : idcharacter});
    ...
    static isDoingRandomStuff (answers_so_far) {
        ...
        for (let val of values)
            statistics[val] = statistics[val] == undefined ? 1 : (statistics[val] + 1);
