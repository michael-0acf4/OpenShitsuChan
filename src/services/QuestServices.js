...
module.exports = class QuestServices {
  ...
  static async getSingleQuestion(
    connexion,
    exclude_question_list,
    chara_in_mind,
  ) {
    const { most_probable_barrier, random_window_max_asking_question } =
      config.constant;
    ...
    if (exclude_question_list.length < random_window_max_asking_question) {
      const res = await QuestServices.getMaxInfosQuestionAccordingToModel(
        connexion,
        exclude_question_list,
      );
      if (res != null) {
    ...
    if (
      chara_in_mind != null && chara_in_mind.probability > most_probable_barrier
    ) {
      const res = await QuestServices.getNextMostProbableQuestion(
        connexion,
        exclude_question_list,
        chara_in_mind,
      );
    ...
    if (exclude_question_list.length > 0) {
      where = 'WHERE "idquestion" NOT IN (' +
        exclude_question_list.map((_, i) => "$" + (i + 1)).join(",") + ")";
    }
   ...
   * @param {JSON} chara
   */
  static async getNextMostProbableQuestion(
    connexion,
    exclude_question_list,
    chara,
  ) {
    ...
    let offset = 2;
    if (exclude_question_list.length > 0) {
      where += ' AND "idquestion" NOT IN (' +
        exclude_question_list.map((_, i) => "$" + (i + offset)).join(",") + ")";
    }
  ...
  static pickProbability(choice_index) {
    ...
    if (index < 1 || index > keys.length) {
      throw Error(
        "Invalid choice index, should get between " +
          [1, keys.length].join("-"),
      );
    }
   ...
   * @param {string[]} exclude_question_list
   */
  static async getMaxInfosQuestionAccordingToModel(
    connexion,
    exclude_question_list,
  ) {
    let where = "";
    ...
    let top = Math.max(
      1,
      Math.floor(list.length / config.constant.top_question_split),
    );
